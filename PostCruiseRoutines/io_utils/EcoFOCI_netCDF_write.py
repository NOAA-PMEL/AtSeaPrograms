...
class CTDbtl_NC(object):
    ...
    def file_create(self):
            rootgrpID = Dataset(self.savefile, CTDbtl_NC.nc_read, format=CTDbtl_NC.nc_format)
    ...
    def sbeglobal_atts(self, raw_data_file='', Water_Mass='B', Water_Depth=9999, Prog_Cmnt='',\
        ...
        self.rootgrpID.INST_TYPE = 'BTL'
        self.rootgrpID.DATA_CMNT = raw_data_file
        self.rootgrpID.EPIC_FILE_GENERATOR = 'btl2nc.py V' + __version__ 
...
class NetCDF_Create(object):
    ...
    def file_create(self):
            rootgrpID = Dataset(self.savefile, NetCDF_Create.nc_read, format=NetCDF_Create.nc_format)
            self.rootgrpID = rootgrpID
            return ( rootgrpID )
        
    def sbeglobal_atts(self, raw_data_file='', Water_Mass='', Water_Depth=9999, 
                       Prog_Cmnt='', Experiment='', Edit_Cmnt='', Station_Name='', 
                       SerialNumber='',Instrument_Type='', History='', Cast='',
                       Barometer=9999, Wind_Dir=999, Wind_Speed=99, Cruise='', coord_system='',
                       Air_Temp=99.9, sfc_extend=''):
        """
        Assumptions
        -----------
    ...
    def variable_init(self, nchandle):
        ...
        for v_name in nchandle.variables.keys():
            print v_name
            if not v_name in ['time','time2','depth','dep','lat','lon','latitude','longitude']:
    ...
    def add_data(self, data=None):
        ...
        for ind, varname in enumerate(data.keys()):
            if not varname in ['time','time2','lat','lon','depth','dep','latitude','longitude']:
