...
# get excel file
W = readXlsx('/Users/bell/in_and_outbox/2016/stabeno/mar/arctic_maps/Arctic\ Mooring\ Locations_2016-7.xlsx', sheet=1)
...
m = Basemap(resolution='i',projection='stere',width=1500000, height=1210000, lat_0=74,lon_0=191)
...
m.drawparallels(np.arange(66,80,2.),labels=[1,0,0,0],color='black',dashes=[1,1],labelstyle='+/-',linewidth=0.2) # draw parallels
...
for index, row in enumerate(W):
    ...
    try:
        ...
        lat_s = float(row['D'])
        lat_e = float(row['F'])
        print ("line {0}: {0} {1} {2} {3} ").format(row['B'], float(row['C']),float(row['D']),float(row['E']),float(row['F']))
        x, y = m([lon_s * -1., lon_e * -1.], [lat_s, lat_e])
        if 'CBWCS' == row['B']:
            m.plot(x, y, '--', markersize=8, linewidth=2, color='r', alpha=0.5)
        else:
            m.plot(x, y, '-', markersize=8, linewidth=2, color='r', alpha=0.75)
        
    except:
        try:
            if 'PMEL Mooring' == row['B']:
                lon_s = float(row['C'])
                lat_s = float(row['D'])
                print ("Mooring Site: {0}").format(row['B'])
                x, y = m(lon_s * -1., lat_s)
                m.plot(x, y, 'o', markersize=6, color='g', alpha=0.75)
            elif 'Mooring' == row['B']:
                lon_s = float(row['C'])
                lat_s = float(row['D'])
                print ("Mooring Site: {0}").format(row['B'])
...
plt.savefig('IBCAO_Chuckchi_AW14_transects_iceextent.png', bbox_inches='tight', dpi = (300))
plt.close()
